...
class VideoProcessor:
        ...
        with torch.no_grad():
            for i in range(0, self.config['max_frames'], self.config['batch_size']):
                ...
                if not batch_data:
                    break
                if(count == 2):
                    batch_results, t_match = self._interpolation_keypoints(Hs, t_match)
                    count = 0
                else:
    ...
    def _interpolation_keypoints(self, Hs, t_match):
        """Interpolate keypoints for a batch of frames."""
        batch_results = []
        print("interpolating")
        idxs = np.arange(len(Hs))
        t_match_start = time.time()
        spl = make_interp_spline(idxs[::2], Hs[::2], k=1, axis=0)  
        input("")
        batch_results = spl(idxs)
        batch_results[:, 0, 2, 2] = int(round(batch_results[:, 0, 2, 2]))
